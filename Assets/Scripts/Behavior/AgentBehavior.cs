// ================================================== //
// DUE TO THE LENGHT OF THIS SCRIPT
// HERE IS THE LIST OF METHODS:
// Use "CTRL + F" then type the method that need to find
// ================================================== //
// private void start()
...
// private void ReExploreOrBestPath()
...
// private void ApplyFreeze(float freezeDuration)
...
public class AgentBehavior : MonoBehaviour
{
    ...
    private Pathfinding pathfinding; //for calculating best path
    [SerializeField] private float exploreProbability = 0.3f; //higher value means more exploration thus radomly choosing nodes to navigate based on pheromone level
    private float pheromoneDepositRate = 0.5f; //higher means more deposite; evaporation of pheromones is ran on the gamegrid script
    private bool isDestroyed = false;
    [SerializeField] private float randomThreshold = 0.3f; // modify for exploring when choosing between nodes that have high pheromone level or disregards pheromone level
    ...
    private IEnumerator Explore()
    {
        ...
        while (true)
        {
            ...
            List<Node> unvisitedWalkableNeighbors = new List<Node>();
            foreach (Node neighbor in neighbors)
            {
                if (neighbor.walkable && !neighbor.visited)
                {
                    unvisitedWalkableNeighbors.Add(neighbor);
                }
            }
            if (unvisitedWalkableNeighbors.Count > 0)
            {
                Node nextNode = null;
                float maxPheromoneLevel = float.MinValue;
                foreach (Node neighbor in unvisitedWalkableNeighbors)
                {
                ...
                {
                    nextNode = unvisitedWalkableNeighbors[Random.Range(0, unvisitedWalkableNeighbors.Count)];
                }

                nextNode.visited = true;
                List<Node> path = pathfinding.FindPath(currentPosition, new Vector2Int(nextNode.x, nextNode.y));
    ...
    private IEnumerator FollowPath(List<Node> path)
    {
        ...
        while (path != null && currentNodeIndex < path.Count)
        {
            ...
            if (HasReachedNextNode(path))
            {
                ...
                path = pathfinding.FindPath(thisPosition, nextPosition);
                currentNodeIndex = 0;
            }
            else
            {
    ...
    private IEnumerator HandleBlockedPaths()
    {
        Vector2Int startPosition = new Vector2Int((int)transform.position.x, (int)transform.position.y);
        Vector2Int targetPosition = GetTargetPosition();
        List<Node> newPath = pathfinding.FindPath(startPosition, targetPosition);
